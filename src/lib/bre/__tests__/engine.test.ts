...
describe("BRE — Co-applicant Employment Type universal exclusion", () => {
  it("omits employment_type from the co-applicant trace", () => {
    const r = evaluate(strongProfile, DEFAULT_SCORING_CONFIG_V1, [lenderRule()]);
    expect(r.buckets.coapplicant.trace.some((t) => t.param_key === "employment_type")).toBe(false);
  });

  it("renormalizes remaining co-applicant weights to ~100", () => {
    const r = evaluate(strongProfile, DEFAULT_SCORING_CONFIG_V1, [lenderRule()]);
    const sum = r.buckets.coapplicant.trace.reduce((s, t) => s + (t.weight ?? 0), 0);
    expect(sum).toBeGreaterThanOrEqual(99.5);
    expect(sum).toBeLessThanOrEqual(100.5);
  });

  it("produces identical co-applicant scores for Salaried vs Business owner / Self-employed when other fields match", () => {
    const salaried = evaluate(
      { ...strongProfile, coapplicant: { ...strongProfile.coapplicant, employment_type: "salaried_private" } },
      DEFAULT_SCORING_CONFIG_V1,
      [lenderRule()],
    );
    const business = evaluate(
      { ...strongProfile, coapplicant: { ...strongProfile.coapplicant, employment_type: "self_employed_business" } },
      DEFAULT_SCORING_CONFIG_V1,
      [lenderRule()],
    );
    const professional = evaluate(
      { ...strongProfile, coapplicant: { ...strongProfile.coapplicant, employment_type: "self_employed_professional" } },
      DEFAULT_SCORING_CONFIG_V1,
      [lenderRule()],
    );
    expect(salaried.buckets.coapplicant.total).toBe(business.buckets.coapplicant.total);
    expect(salaried.buckets.coapplicant.total).toBe(professional.buckets.coapplicant.total);
  });

  it("does not surface employment_type in rejection reasons or as a missing-data blocker", () => {
    const r = evaluate(
      { ...strongProfile, coapplicant: { ...strongProfile.coapplicant, employment_type: null as any } },
      DEFAULT_SCORING_CONFIG_V1,
      [lenderRule()],
    );
    expect(r.rejection_reasons.every((s) => !/employment/i.test(s))).toBe(true);
    expect(r.buckets.coapplicant.trace.some((t) => t.param_key === "employment_type")).toBe(false);
  });

  it("applies to legacy DB-stored configs that still list employment_type (filtered before scoring)", () => {
    const legacyCfg = {
      ...DEFAULT_SCORING_CONFIG_V1,
      coapplicant_params: [...DEFAULT_SCORING_CONFIG_V1.coapplicant_params],
    };
    // employment_type is already present in defaults — simulate legacy by evaluating directly.
    const r = evaluate(strongProfile, legacyCfg as any, [lenderRule()]);
    expect(r.buckets.coapplicant.trace.some((t) => t.param_key === "employment_type")).toBe(false);
    const sum = r.buckets.coapplicant.trace.reduce((s, t) => s + (t.weight ?? 0), 0);
    expect(sum).toBeGreaterThanOrEqual(99.5);
    expect(sum).toBeLessThanOrEqual(100.5);
