-- Remove the hardcoded 10-applied edit cap from submit_edit_request and decide_edit_request.
-- All other guards (pending-request, reason length, RLS, whitelist, terminal-stage, dup checks) are preserved.
...
BEGIN
  ...
  FOR v_key IN SELECT unnest(_approved_fields) LOOP
    ...
    IF v_req.requested_changes ? v_key THEN
      ...
      IF v_key IN ('coapplicant_relation','coapplicant_employment_type') THEN v_coapp_changed := true; END IF;
  ...
  IF v_applied ? 'pincode' THEN
    v_pin_norm := regexp_replace(COALESCE(v_applied ->> 'pincode',''), '\D', '', 'g');
    IF length(v_pin_norm) = 6 THEN
      SELECT * INTO v_pin_row FROM public.pincodes_master WHERE pincode = v_pin_norm LIMIT 1;
      IF v_pin_row.pincode IS NOT NULL THEN
        v_applied := v_applied
          || jsonb_build_object('city', v_pin_row.city)
