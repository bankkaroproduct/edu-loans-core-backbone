-- Fix: student_full_name is a GENERATED column. Remove it from the whitelist
-- and from the dynamic UPDATE in decide_edit_request. The DB derives it automatically.

CREATE OR REPLACE FUNCTION public.submit_edit_request(_lead_id uuid, _changes jsonb, _reason text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR v_key IN SELECT unnest(_approved_fields) LOOP
    -- Skip generated column defensively
  ...
  -- NOTE: student_full_name is a GENERATED column derived from first+last by the DB,
  -- so we never write to it. No client-side derivation needed.
...
-- Mirror fix in the QA helper so we can re-run scenario 3
CREATE OR REPLACE FUNCTION public._qa_decide_edit_request(
  _actor_auth_id uuid, _request_id uuid, _action text,
  _approved_fields text[] DEFAULT NULL, _decision_note text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE
  v_actor_id uuid; v_actor_role app_role;
  v_req lead_edit_requests%ROWTYPE; v_lead student_leads%ROWTYPE;
  v_applied jsonb := '{}'::jsonb; v_key text; v_val jsonb;
  v_set_clauses text := '';
  v_trigger_fields text[] := ARRAY['intended_study_country','university_name_raw','course_name','course_category','intake_term','intake_year','loan_amount_required','collateral_available','collateral_notes','coapplicant_income','coapplicant_existing_emi','coapplicant_employment_type'];
  v_recalc boolean := false; v_coapp_changed boolean := false;
  v_norm_phone text; v_norm_email text; v_dup_lead_id uuid; v_sql text;
BEGIN
  SELECT id, role INTO v_actor_id, v_actor_role FROM public.users WHERE auth_user_id = _actor_auth_id LIMIT 1;
  IF v_actor_id IS NULL THEN
    SELECT id, role INTO v_actor_id, v_actor_role FROM public.users WHERE id = _actor_auth_id LIMIT 1;
  END IF;
  IF v_actor_role NOT IN ('admin','super_admin') THEN RAISE EXCEPTION 'forbidden: admin role required'; END IF;
  ...
  IF _action = 'reject' THEN
    IF _decision_note IS NULL OR length(trim(_decision_note)) < 10 THEN
      RAISE EXCEPTION 'rejection requires a note of at least 10 characters'; END IF;
    UPDATE public.lead_edit_requests SET status='rejected', admin_decision_note=trim(_decision_note),
      decided_by_user_id=v_actor_id, decided_at=now() WHERE id=_request_id;
    RETURN jsonb_build_object('ok', true, 'request_id', _request_id, 'status', 'rejected');
  END IF;

  IF _action <> 'approve' THEN RAISE EXCEPTION 'invalid action: %', _action; END IF;
  IF _approved_fields IS NULL THEN RAISE EXCEPTION 'approval requires at least one approved field'; END IF;
  ...
  FOR v_key IN SELECT unnest(_approved_fields) LOOP
    ...
    IF v_req.requested_changes ? v_key THEN
      v_val := v_req.requested_changes -> v_key;
      IF v_key IN ('student_phone','student_whatsapp','coapplicant_mobile') AND jsonb_typeof(v_val)='string' THEN
        v_norm_phone := public.normalize_phone(v_val #>> '{}'); v_val := to_jsonb(v_norm_phone);
        IF v_key = 'student_phone' AND v_norm_phone IS NOT NULL THEN
          SELECT id INTO v_dup_lead_id FROM public.student_leads
          WHERE partner_id=v_lead.partner_id AND student_phone=v_norm_phone AND is_archived=false AND id<>v_lead.id LIMIT 1;
          IF v_dup_lead_id IS NOT NULL THEN
            RAISE EXCEPTION 'duplicate_after_normalization: phone % already exists on lead %', v_norm_phone, v_dup_lead_id;
          END IF;
        END IF;
  ...
  UPDATE public.lead_edit_requests
  SET status='applied', applied_changes=v_applied, admin_decision_note=_decision_note,
      decided_by_user_id=v_actor_id, decided_at=now(), applied_at=now()
  WHERE id=_request_id;
