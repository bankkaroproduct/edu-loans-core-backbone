-- 1. Index to speed up applied-count lookups per lead
CREATE INDEX IF NOT EXISTS idx_lead_edit_requests_lead_status
  ON public.lead_edit_requests (lead_id, status);

-- 2. submit_edit_request: allow empty diff + enforce 10-applied cap
...
BEGIN
  ...
  -- 10-limit: only count applied requests that actually modified fields
  ...
  -- NOTE: empty v_filtered is now allowed (review-only request)
  ...
  VALUES
    ('lead_edit_request', v_request_id,
     CASE WHEN v_filtered = '{}'::jsonb THEN 'edit_request_created_review_only' ELSE 'edit_request_created' END,
     v_actor_id, 'partner_agent',
  ...
  VALUES
    ('system_alert', 'admin', 'lead_edit_request', v_request_id,
     CASE WHEN v_filtered = '{}'::jsonb
          THEN 'New review-only lead request submitted'
          ELSE 'New lead edit request submitted for review' END);

  RETURN jsonb_build_object('ok', true, 'request_id', v_request_id, 'review_only', v_filtered = '{}'::jsonb);
END;
$function$;

-- 3. decide_edit_request: support acknowledge path for empty-diff requests + recheck 10-limit on approve
...
BEGIN
  ...
  -- ============ ACKNOWLEDGE PATH (review-only request) ============
  ...
  -- ============ STANDARD APPROVE PATH (with field changes) ============
  ...
  -- Recheck 10-limit at approve time (race protection)
  SELECT count(*) INTO v_applied_count
  FROM public.lead_edit_requests
